    ...
    def compute(self):
        ...
        for _, row in df_merged.iterrows():
            ...
            for _, p in df_price.iterrows():
                if p["CMS category"] == row["cat_level_2_and_3"] and p["Weightclass"] == row["weight"]:
                    buyer_seller_key = row["buyer_country-seller_country"]
                    if row["order_creation_date"] < change_price_date:
                        key = f"{buyer_seller_key}-OLD"
                    else:
                        key = f"{buyer_seller_key}-libero_logistics"
        ...
        if has_de:
            ...
            df_merged["price"] = np.where(df_merged["price"] != 0, df_merged["price"], df_merged["price_de"])
        ...
        return df_merged[["Order ID", "buyer_country-seller_country", "weight", "price", "price_libero_logistics", "Delta", "Delta_sum"]], delta_sum, flag
    ...
    def _get_germany_prices(self, df):
        ...
        for _, row in df.iterrows():
            ...
            for _, price_row in df_price_de.iterrows():
                if price_row["CMS category"] == category:
